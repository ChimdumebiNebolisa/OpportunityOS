...
| Stale or fabricated personal synchronization | Independent source controls, capability states, source cursors/high-water marks, run limits, read-only adapters, and explicit `GATED` failures | Provider APIs may change; unsupported Gmail/ChatGPT paths remain unavailable until a real capability is installed and verified. |
| Untrusted personal-source assertions | Candidate-before-canonical flow, subject/assertion classification, provenance, content fingerprints, exact-agreement silence, material review batches, and temporal fields | A human can still approve an incorrect candidate; review source evidence and timestamps. |
